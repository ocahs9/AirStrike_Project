...
public class UIController : MonoBehaviour
{
    ...
    [SerializeField] AlertUIController alertUIController;

    [Header("1st-3rd View Control")]
    [SerializeField] RectTransform commonCenterUI;
    [SerializeField] RectTransform firstCenterViewTransform;
    [SerializeField] RectTransform thirdCenterViewTransform;
    
    [SerializeField] Canvas firstViewCanvas;
    [SerializeField] Vector2 firstViewAdjustAngle;

    [Header("1st View Center UI")]
    [SerializeField] UVController speedUV;
    [SerializeField] UVController altitudeUV;

    [SerializeField] Image throttleGauge;
    [SerializeField] HeadingUIController headingUIController;
    ...
    [SerializeField] TextMeshProUGUI mslText;
    [SerializeField] TextMeshProUGUI spwText;

    [SerializeField] TextMeshProUGUI flrText;

    [SerializeField] TextMeshProUGUI dmgText;

    [SerializeField] GameObject mslIndicator;
    [SerializeField] GameObject spwIndicator;

    // Status
    [Header("Lower Right UI : Aircraft/Weapon Status")]
    [SerializeField] Image aircraftImage;
    [SerializeField] CooldownImage leftMslCooldownImage;
    [SerializeField] CooldownImage rightMslCooldownImage;

    [Header("Minimap Misc.")]
    [SerializeField] MinimapCompass minimapCompass;
    ...
    [Header("Sounds")]
    [SerializeField] AudioClip spwChangeAudioClip;
    [SerializeField] AudioClip mslChangeAudioClip;
    [SerializeField] AudioClip timeLowAudioClip;
    ...
    {
        this.remainingTime = (float)remainingTime;
    }
    ...
    void SetTime()
    {
        ...
        if(OnRedTimer == false && remainingTime < 10 && LowTime == false)
        {
        ...
        int min = seconds / 60;
        int sec = seconds % 60;
        int millisec = (int)((remainingTime - seconds) * 100);
    ...
    {
        if(OnRedTimer == true || GameManager.Instance.IsGameOver == true)
        {
            CancelInvoke();
            return;
        }
    ...
    {
        this.scores = scores;
        string text = string.Format("SCORE <mspace=18>{0}</mspace>", this.scores);
        scoreText.text = text;
    }
    ...
    void Update()
    {
        if(enableCount == true && remainingTime > 0 && GameManager.Instance.IsGameOver == false) SetTime();
    }
}
